...
class RegisterView(APIView):
    def post(self, request):
        serializer = serializers.RegisterSerialisers(data=request.data)
...
def LoginAPIView(APIView):
    def post(self, request):
        ...
        return  Response(status=status.HTTP_401_UNAUTHORIZED, data={'massege': 'Invalid credentials'})
    ...
    class SMSCodeConfirm(APIView):
        def post(self, request):
            ...
            try:
                ...
                return Response(status=status.HTTP_404_NOT_FOUND, data={'massage': 'Code not found'})
